        ...
        <Box sx={{ width: '50%' }}>
          <Controller
            ...
            rules={{
              ...
              maxLength: { value: 40, message: 'Длина строки должна быть не больше 40 символов' },
            }}
            render={({ field }) => (
              <TextField
                fullWidth
          ...
          <Controller
            ...
            control={control}
            disabled={loading}
            render={({ field }) => <TextField multiline fullWidth rows={4} size="small" label="Описание" {...field} />}
          />
        </Box>
      </Paper>
      {/* TODO move to sep footer component */}
      <Box sx={{ mt: 3 }}>
        <Button variant="contained" disabled={loading} onClick={() => handleSubmit(onSubmit)()} sx={{ mr: 3 }}>
          Создать
        </Button>
        <Button variant="outlined" onClick={handleMoveToCategories} disabled={loading}>
          Отмена
        </Button>
      </Box>
    </>
